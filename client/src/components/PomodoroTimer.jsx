  ...
  useEffect(() => {
    ...
    if (isRunning) {
      interval = setInterval(() => {
        setSecondsLeft((prev) => {
          if (prev === 0) {
            const nextTime = isFocusTime ? breakMinutes * 60 : focusMinutes * 60;
    ...
    <div className="bg-white/15 backdrop-blur-lg border border-white/20 text-white rounded-3xl p-6 shadow-xl max-w-xl mx-auto">
      ...
      <div className="flex justify-center gap-4">
        <button
          onClick={handleStart}
          disabled={isRunning}
          className="px-6 py-2 rounded-xl bg-green-500 hover:bg-green-600 text-white font-semibold shadow-md transition"
        >
          Start
        </button>
        <button
