...
const PomodoroTimer = ({ compact = false }) => {
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-white/10 backdrop-blur-md text-white border border-white/20 rounded-full shadow-md gap-4 font-mono">
        {isRunning ? (
          <span className="text-sm font-semibold whitespace-nowrap">
            {isFocusTime ? "FOCUS" : "BREAK"} {formatTime(secondsLeft)}
          </span>
        ) : (
          <input
            type="text"
            ...
            className="w-24 text-center rounded-md bg-white/10 text-white border border-white/20 text-sm outline-none"
          />
        )}
  ...
  // Full UI (unchanged except for toggle logic)
  return (
